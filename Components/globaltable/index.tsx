...
import { Column } from "../../utils/table-types/column-type";
...
interface TableProps<T extends { [key: string]: any }> {
  ...
  disableRowCheckbox?: (row: T) => boolean; // optionally disable selection per-row
}
...
export default function GlobalTable<T extends { [key: string]: any }>({
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    const selectableRows = rows.filter((row) => !(disableRowCheckbox?.(row)));
    let newSelected: any[] = [];
    if (event.target.checked) {
      newSelected = selectableRows.map((row) => encryptAES(row[rowKey]));
    }
    setSelected(newSelected);
    onSelectionChange?.(newSelected);
  };
              ...
              {enableCheckbox && (
                ...
                >
                  <Checkbox
                    ...
                    indeterminate={(() => {
                      const selectableRows = rows.filter((row) => !(disableRowCheckbox?.(row)));
                      return selected.length > 0 && selected.length < selectableRows.length;
                    })()}
                    checked={(() => {
                      const selectableRows = rows.filter((row) => !(disableRowCheckbox?.(row)));
                      return selectableRows.length > 0 && selected.length === selectableRows.length;
                    })()}
              ...
              return (
                <TableRow
                  ...
                  onClick={() => {
                    rest.onRowClick?.(row);
                    if (url) {
                      navigate.push(`${url}?id=${encryptAES(id)}`);
                    }
                  }}
